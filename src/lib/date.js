export function getSimpleDate(dateObject) {
    return { 
...
export function getDateObject(simpleDate) {
    return new Date(
        simpleDate.year,
        simpleDate.month - 1,
        simpleDate.day
...
export function isDateUpcoming(date) {
    return getTimeDifference(date, getNow()) > 0;
}
...
export function sortByDate(items, options) {
    ...
    futureItems.sort((a, b) => a.time - b.time);
    pastItems.sort((a, b) => b.time - a.time);
